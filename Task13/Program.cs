    ...
    namespace TPL
    {
        class Program
        {
            static void MyTask(object arg)
            {
                ...
                // Если задача сразу после старта отменена - выбросить OperationCanceledException.
                ...
                for (int i = 0; i < 80; i++)
                {
                    if (token.IsCancellationRequested) // Задача отменена?
                    {
                        ...
                        token.ThrowIfCancellationRequested(); // выбросить OperationCanceledException.
                    }
